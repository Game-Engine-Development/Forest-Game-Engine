...
struct ImGuiPlatformIO;             // Multi-viewport support: interface for Platform/RenderSystem back-ends + viewports to render
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasViewports  = 1 << 12   // Back-end RenderSystem supports multiple viewports.
};
...
struct ImGuiPlatformIO
{
    ...
    //
    // The functions are designed so we can mix and match 2 imgui_impl_xxxx files, one for the Platform (~window/input handling), one for RenderSystem.
    // Custom engine back-ends will often provide both Platform and RenderSystem interfaces and so may not need to use all functions.
    // Platform functions are typically called before their RenderSystem counterpart, apart from Destroy which are called the other way.
    ...
    int     (*Platform_CreateVkSurface)(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface); // (Optional) For a Vulkan RenderSystem to call into Platform code (since the surface creation needs to tie them both).

    // (Optional) RenderSystem functions (e.g. DirectX, OpenGL, Vulkan)
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoRendererClear          = 1 << 5,   // Platform Window: RenderSystem doesn't need to clear the framebuffer ahead (because we will fill it entirely).
...
struct ImGuiViewport
{
    ...
    // Our design separate the RenderSystem and Platform back-ends to facilitate combining default back-ends with each others.
    // When our create your own back-end for a custom engine, it is possible that both RenderSystem and Platform will be handled
