...
void main()
{
    ...
    */
    ...
    vec2 tiledCoord = texCoord;
    if(true) {
        tiledCoord = texCoord * 20;
    }

    vec3 albedo = texture(albedoMap, tiledCoord).rgb;
    float roughness = texture(roughnessMap, tiledCoord).r;
    float metallic = 0;
    float ao = 1;
    //vec3 normal = texture(normalMap, tiledCoord).rgb;
